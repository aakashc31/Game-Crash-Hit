...
void initRendering(void)
{
    ...
    // loading textures
    Image* image = loadBMP("img/glossy.bmp");
    _textureId = loadTexture(image);

    image = loadBMP("img/floor2.bmp");
    floor_id = loadTexture(image);

    image = loadBMP("img/metallic.bmp");
    _metalId = loadTexture(image);

    image = loadBMP("img/walltile.bmp");
    _wallId = loadTexture(image);

    image = loadBMP("img/start2.bmp");
    _startId = loadTexture(image);

    image = loadBMP("img/exit2.bmp");
...
void initVars(){
    ...
    clicks = 0;
    z_global = 0;
    angX = 0, angZ = 0, angY = 0; //this is for rotation of view port, for debugging purposes
    printscore = false;
    score = 0;
    global_size = 1000;
    mypause = false;
...
void drawScene()
{
    ...
    if(endingGame)
    {
        glPushMatrix();
        glScalef(2,2,2);
        drawText("Game Over :D", 375, 800);
        drawText("Press Esc", 400,700);
        glPopMatrix();
    }
